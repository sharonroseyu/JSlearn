...
### 浏览器存储
> Cookie，SessionStorage，LocalStorage这三者都可以被用来在浏览器端存储数据，而且都是字符串类型的键值对。【另外还有一种存储模式叫session，这种级别的存储属于服务端绘画级别的存储。】
#### cookie 概念和作用
  - 1. cookie 翻译过来的意思是小甜饼的意思，是网景公司的前雇员Lou Montulli 在1993年3月发明的。
  - 2. cookie 是纯文本格式，不包含任何可执行的代码信息，伴随着用户在web服务器和浏览器之间传递。
  - 3. cookie本质上属于http的范畴，因为http协议本身是无状态的，服务器是没有办法区分请求来自于那个客户端，几遍来自同一个客户端的多次请求，我们的服务端也是没有办法来区分的。就是因为http协议是无状态的，所以才需要cookie去维护客户端的状态。

##### cookie的生成方式
  - 1. 客户端生成：在javascript中通过document.cookie属性，可以创建维护和删除cookie,设置cookie属性值并不会删除咋页面中所有的cookie。它只是简单的创建或修改字符串指定的cookie，要是用javascript提取cookie的值，只需要使用document.cookie中读取即可。
  - 2. 服务端生成：web服务器通过发送一个称为 Set-cookie 的http消息头来创建一个cookie，httpOnly为避免跨域脚本(XSS)攻击，通过javascript的document.cookie API 无法访问HttpOnly 标记的cookie。

##### cookie的缺点：
  - 安全性：由于cookie在http中是明文传递的，其中包含的数据都可以被他人访问，可能会被篡改和盗用。
  - 大小限制：cookie的大小限制在4kb左右，若要大量存储显然不是理想的选择。
  - 增加流量：cookie每次访问都会被自动添加到Requeset Header中，无形增加了流量。